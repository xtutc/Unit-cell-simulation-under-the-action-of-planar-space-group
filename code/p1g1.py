...
# 定义 p1g1 平面群的所有对称操作（旋转矩阵和平移向量）
p1g1_operations = [
    # 1. 恒等操作
    {'rotation': np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
    # 2. 镜面反射 (x → -x) + 平移 (0, 1/2)
    {'rotation': np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]), 'translation': np.array([0, 0.5, 0])},
]

def p1g1_generate_atoms_structure(cell: np.ndarray,
    ...
    # 对每个原子随机挑选一个对称操作进行应用
    for i in range(n):
        op = random.choice(p1g1_operations)
        R = op['rotation']
        t = op['translation']
        # 应用旋转和平移
