...
p31m_operations = [
    ...
    # 5. (x → -x) + (y → -x+y)
    {'rotation': np.array([[-1, 0, 0], [-1, 1, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
    # 6. (x → x-y) + (y → -y)
    {'rotation': np.array([[1, -1, 0], [0, -1, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
]

def p31m_generate_atoms_structure(cell: np.ndarray,
    ...
    # 对每个原子随机挑选一个对称操作进行应用
    for i in range(n):
        op = random.choice(p31m_operations)
        R = op['rotation']
        t = op['translation']
        # 应用旋转和平移
