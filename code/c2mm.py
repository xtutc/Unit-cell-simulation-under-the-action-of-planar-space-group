...
c2mm_operations = [
    ...
    {'rotation': np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
    # 5. 平移 (1/2, 1/2)
    {'rotation': np.eye(3), 'translation': np.array([0.5, 0.5, 0])},
    # 6. 180度旋转 + 平移 (1/2, 1/2)
    {'rotation': np.array([[-1, 0, 0], [0, -1, 0], [0, 0, 1]]), 'translation': np.array([0.5, 0.5, 0])},
    # 7. 镜面反射 (x → -x) + 平移 (1/2, 1/2)
    {'rotation': np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]), 'translation': np.array([0.5, 0.5, 0])},
    # 8. 镜面反射 (y → -y) + 平移 (1/2, 1/2)
    {'rotation': np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]]), 'translation': np.array([0.5, 0.5, 0])},
]

def c2mm_generate_atoms_structure(cell: np.ndarray,
    ...
    # 对每个原子随机挑选一个对称操作进行应用
    for i in range(n):
        op = random.choice(c2mm_operations)
        R = op['rotation']
        t = op['translation']
        # 应用旋转和平移
