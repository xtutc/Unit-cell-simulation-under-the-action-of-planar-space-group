...
# 定义 p4gm 平面群的所有对称操作（旋转矩阵和平移向量）
p4gm_operations = [
    # 1. 恒等操作
    {'rotation': np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
    # 2. 180度旋转 (绕原点)
    {'rotation': np.array([[-1, 0, 0], [0, -1, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
    # 3. 关于y轴的镜面反射 (x → -x) + 平移 (1/2, 1/2)
    {'rotation': np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]]), 'translation': np.array([0.5, 0.5, 0])},
    # 4. 关于x轴的镜面反射 (y → -y) + 平移 (1/2, 1/2)
    {'rotation': np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]]), 'translation': np.array([0.5, 0.5, 0])},
    # 5. (x → -y) + (y → x)
    {'rotation': np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
    # 6. (y → -x) + (x → y)
    {'rotation': np.array([[0, 1, 0], [-1, 0, 0], [0, 0, 1]]), 'translation': np.array([0, 0, 0])},
    # 7. (x → -y) + (y → -x) + 平移 (1/2, 1/2)
    {'rotation': np.array([[0, -1, 0], [-1, 0, 0], [0, 0, 1]]), 'translation': np.array([0.5, 0.5, 0])},
    # 8. (y → x) + (x → y) + 平移 (1/2, 1/2)
    {'rotation': np.array([[0, 1, 0], [1, 0, 0], [0, 0, 1]]), 'translation': np.array([0.5, 0.5, 0])},
]

def p4gm_generate_atoms_structure(cell: np.ndarray,
    ...
    # 对每个原子随机挑选一个对称操作进行应用
    for i in range(n):
        op = random.choice(p4gm_operations)
        R = op['rotation']
        t = op['translation']
        # 应用旋转和平移
